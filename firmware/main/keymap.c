...
static uint8_t mouse_speed = KEYMAP_SPEED_DEFAULT;
...
{
    if (length != KEYMAP_LENGTH || memcmp(p, "KM\x03\x0e", 4) || p[6] != KEYMAP_MODES ||
        p[7] < 1 || p[7] > KEYMAP_SPEED_MAX) { return false; }
    for (unsigned m = 0; m < KEYMAP_MODES; ++m) {
        for (unsigned i = 0; i < KEYMAP_COUNT; ++i) {
            ...
            if ((v & 0xe000) || !valid_entry(&e) || (e.button == 0x6a && v) ||
                (m == 1 && e.button == 0x4a && v)) { return false; }
            out[m][i] = e;
        }
    }
    return true;
...
void keymap_init(void)
{
    ...
    entries[1][10] = (keymap_entry_t){0x4a, KEYMAP_NONE, 0, 0};
    mouse_speed = KEYMAP_SPEED_DEFAULT;
    revision = 0;
    uint8_t p[KEYMAP_LENGTH];
    keymap_entry_t loaded[KEYMAP_MODES][KEYMAP_COUNT];
    if (!keymap_storage_load(p)) { return; }
    if (!memcmp(p, "KM\x02\x0e", 4) && !p[7]) {
        /* Keep both old profiles; HOME is now reserved for repositioning. */
        p[2] = 3; p[7] = KEYMAP_SPEED_DEFAULT;
        p[56] = p[57] = 0;
    }
    if (decode(p, sizeof(p), loaded)) {
        ...
        loaded[1][10] = (keymap_entry_t){0x4a, KEYMAP_NONE, 0, 0};
        memcpy(entries, loaded, sizeof(entries));
    } else { return; }
    revision = p[4] | (p[5] << 8);
}
...
uint8_t keymap_mouse_speed(void) { return mouse_speed; }

void keymap_read(uint8_t p[KEYMAP_LENGTH])
{
    memcpy(p, "KM\x03\x0e", 4);
    p[4] = revision & 0xff; p[5] = revision >> 8; p[6] = KEYMAP_MODES; p[7] = mouse_speed;
    for (unsigned m = 0; m < KEYMAP_MODES; ++m) {
        for (unsigned i = 0; i < KEYMAP_COUNT; ++i) {
...
int keymap_apply(const uint8_t *p, size_t length)
{
    ...
    memcpy(entries, loaded, sizeof(entries)); revision = next; mouse_speed = p[7];
    return KEYMAP_OK;
}
