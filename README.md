...
새 앱·펌웨어는 버튼별 영문 문자열, 대문자·기호, Enter·Tab, 랜덤 문자 간격과 중간 대기, 최대 100회 반복을 지원합니다. **문자열 매크로** 탭은 연결하거나 버튼을 선택하면 장치 설정을 자동으로 읽으며, 버튼별 편집 내용은 이동·재연결에도 유지합니다. 편집 후 저장하면 앱을 종료해도 리모컨으로 실행할 수 있습니다. 새 USB 제품명은 `USB Keyboard & Mouse`입니다. 사용 방법과 입력 배열 조건은 [매크로 안내](python/README.md#문자열-매크로)를 참고하세요.
