...
**문자열 매크로**는 장치를 연결하거나 모드·버튼을 선택하면 해당 버튼의 설정을 자동으로 읽습니다. 읽는 동안 로딩 상태를 표시하며, 실패하면 **다시 시도**로 재확인할 수 있습니다. **버튼 설정**의 **문자열 매크로 편집…**으로 해당 버튼을 바로 선택할 수도 있습니다. 새 펌웨어가 필요하며, 구형 펌웨어에서는 기존 키 설정을 계속 사용할 수 있습니다.
...
- 버튼별로 저장하지 않은 편집 내용을 앱이 열린 동안 보관합니다. 버튼 이동·연결·재연결 시 자동으로 장치 설정을 읽어도 작성 중인 내용은 유지합니다. 편집 내용을 버리려면 **장치 설정으로 되돌리기**를 누르고 확인하세요. 읽기에 성공한 뒤에만 편집 내용을 교체합니다.
- 저장 실패 시 편집 내용을 유지합니다. **다시 시도** 또는 자동 재연결로 장치 설정을 확인한 뒤 명시적으로 다시 저장하세요. 설정을 읽는 중이거나 읽기에 실패한 상태에서는 저장할 수 없습니다.
