...
Windows가 제공한 HID 목록에는 Bluetooth 설정용 `FF00/1` 컬렉션이 `OK`로 등록돼 있었지만, 기존 앱은 제품명·제조사 일치에 실패하면 해당 장치를 제외했습니다. PnP 등록만으로 HIDAPI 열거 성공이나 제품명 반환값을 확정할 수는 없습니다.

앱 검색은 원본 메타데이터와 제외 이유를 유지합니다. 기존 이름 식별에 더해 HIDAPI가 Bluetooth로 확인한 `FF00/1`을 **확인 필요** 후보로 표시하고, 선택 후 설정 읽기로 프로토콜 호환성을 검증합니다. 검색 중 장치 설정을 읽거나 쓰지 않으며 USB의 동일 Usage 장치는 미확인 후보에서 제외합니다. 펌웨어·페어링·저장 형식은 바꾸지 않습니다.

GUI의 **검색 진단 저장**과 exe의 `--diagnostics <파일 경로>`로 검색 전후 정보 및 빌드·런타임 버전을 저장할 수 있습니다. 사용법과 종료 코드는 [앱 README](../python/README.md#windows-기기-검색과-진단-저장)를 참고하세요. 네 플랫폼 패키지 검사에는 GUI를 시작하지 않는 진단 명령과 빌드 식별자 검증을 포함합니다. CI에서는 장치 0개를 허용합니다.

실제 증상 PC에서 후보 표시, 설정 읽기, 사용자가 요청한 저장과 readback이 확인되기 전에는 해결 완료로 판정하지 않습니다. 원본 열거에서 장치가 누락되면 Windows API 직접 검색을 검토하는 2차 조사 대상이며, 이름 필터 완화만으로 해결됐다고 기록하지 않습니다.
